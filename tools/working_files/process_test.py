...
casename = [r's0_v2_5MW']

# case = f's0_v0'

for case in casename:

    file2read = netCDF4.Dataset(f'/anvil/scratch/x-smata/wrf_les_sweep/runs/gad_sweep/{case}/wrfout_d02_0001-01-01_00_10_00','r',mmap=False) # type: ignore # Read Netcdf-type WRF output file
    ...
    tower_xloc = 386.0                                # Tower x-position in meters
    ...
    vxy_1D  = vx_1D[:,:,lon_dist_0D]  + ( vx_1D[:,:,lon_dist_0D+1]  - vx_1D[:,:,lon_dist_0D]  )*(dist_lon_0D - lon_dist_0D*dy)/dy
    vxy_3D  = vx_3D[:,:,lon_dist_0D]  + ( vx_3D[:,:,lon_dist_0D+1]  - vx_3D[:,:,lon_dist_0D]  )*(dist_lon_0D - lon_dist_0D*dy)/dy
    vxy_6D  = vx_6D[:,:,lon_dist_0D]  + ( vx_6D[:,:,lon_dist_0D+1]  - vx_6D[:,:,lon_dist_0D]  )*(dist_lon_0D - lon_dist_0D*dy)/dy
    vxy_9D  = vx_9D[:,:,lon_dist_0D]  + ( vx_9D[:,:,lon_dist_0D+1]  - vx_9D[:,:,lon_dist_0D]  )*(dist_lon_0D - lon_dist_0D*dy)/dy
    vxy_12D = vx_12D[:,:,lon_dist_0D] + ( vx_12D[:,:,lon_dist_0D+1] - vx_12D[:,:,lon_dist_0D] )*(dist_lon_0D - lon_dist_0D*dy)/dy

    vxyt_0D  = np.mean(vxy_0D,axis=0)/uinf
    vxyt_1D  = np.mean(vxy_1D,axis=0)/uinf
    vxyt_3D  = np.mean(vxy_3D,axis=0)/uinf
    vxyt_6D  = np.mean(vxy_6D,axis=0)/uinf
    vxyt_9D  = np.mean(vxy_9D,axis=0)/uinf
    vxyt_12D = np.mean(vxy_12D,axis=0)/uinf
    ...
    var_holder['vxyt_1D'] = vxyt_1D
    var_holder['vxyt_3D'] = vxyt_3D
    var_holder['vxyt_6D'] = vxyt_6D
    var_holder['vxyt_9D'] = vxyt_9D
    var_holder['vxyt_12D'] = vxyt_12D
